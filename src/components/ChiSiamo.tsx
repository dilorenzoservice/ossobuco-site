...
export default function ChiSiamo({ lang }: Props) {
  ...
  // Placeholder states per le immagini principali
  const [foundersLoaded, setFoundersLoaded] = useState(false);
  const [tboneLoaded, setTboneLoaded] = useState(false);
            ...
            <motion.div key="fondatori"
              ...
              <div className="relative max-w-2xl mx-auto mb-16">
                <div className="relative overflow-hidden rounded-sm">
                  {/* Placeholder */}
                  {!foundersLoaded && (
                    <div className="absolute inset-0 bg-osso-charcoal animate-pulse rounded-sm z-10" />
                  )}
                  <img
                    src="/images/fondatori-emanuele-francesca.webp"
                    alt="Emanuele e Francesca, fondatori di Ossobuco"
                    className="w-full h-[260px] sm:h-[360px] md:h-[460px] object-cover"
                    width={800} height={500}
                    loading="eager" decoding="async"
                    data-src="/images/fondatori-emanuele-francesca.webp"
                    onLoad={() => setFoundersLoaded(true)}
                    onError={onImgError}
                  />
          ...
          {activeTab === "premi" && (
            <motion.div key="premi"
              ...
              <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
                ...
                {/* ─── CARD 1 — Crunchy Competition 2026 ─── */}
                <motion.div
                  className="bg-osso-charcoal/30 rounded-sm warm-border relative overflow-hidden"
                  initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                  <div className="absolute top-0 left-0 w-full h-[2px]" style={{ background: "#CC0000" }} />
                  <div className="p-6 md:p-7">
                    <div className="flex items-center gap-3 mb-4">
                      <span className="text-2xl">🏆</span>
                      <span className="bg-osso-red/15 text-osso-red text-[10px] font-bold uppercase tracking-[0.2em] px-2.5 py-1 rounded-sm">
                        {t.premi.list[0].anno}
                      </span>
                    </div>
                    <h4 className="font-display text-2xl font-bold text-osso-parchment mb-1">{t.premi.list[0].nome}</h4>
                    <p className="text-osso-red font-bold text-base mb-1">{t.premi.list[0].posizione}</p>
                    <p className="text-osso-amber/80 text-xs uppercase tracking-[0.15em] mb-5">
                      {lang === "it" ? "Panino" : "Sandwich"}: {t.premi.list[0].piatto}
                    </p>

                    {/* 3-column photo grid */}
                    <div className="grid grid-cols-3 gap-1.5 mb-5">
                      {[
                        { src: "/images/crunchy-competition.webp", alt: "Crunchy Competition 2026" },
                        { src: "/images/black-dragon.webp",        alt: "Black Dragon burger" },
                        { src: "/images/presentazione-blackdragon.webp", alt: "Presentazione Black Dragon" },
                      ].map(({ src, alt }) => (
                        <div key={src} className="overflow-hidden rounded-sm">
                          <img
                            ...
                            data-src={src} onError={onImgError}
                          />
                        </div>
                      ))}
                    </div>

                    <p className="text-osso-parchment/60 text-sm leading-relaxed">{t.premi.list[0].desc}</p>
                  </div>
                </motion.div>

                {/* ─── CARD 2 — World Steak Challenge 2022 ─── */}
                <motion.div
                  className="bg-osso-charcoal/30 rounded-sm warm-border relative overflow-hidden"
                  initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
                  <div className="absolute top-0 left-0 w-full h-[2px]" style={{ background: "#C8860A" }} />
                  <div className="p-6 md:p-7">
                    <div className="flex items-center gap-3 mb-4">
                      <span className="text-2xl">🌍</span>
                      <span className="bg-osso-amber/20 text-osso-amber text-[10px] font-bold uppercase tracking-[0.2em] px-2.5 py-1 rounded-sm">
                        {t.premi.list[1].anno}
                      </span>
                    </div>
                    <h4 className="font-display text-2xl font-bold text-osso-parchment mb-1">{t.premi.list[1].nome}</h4>
                    <p className="text-osso-red font-bold text-base mb-1">{t.premi.list[1].posizione}</p>
                    <p className="text-osso-amber/80 text-xs uppercase tracking-[0.15em] mb-5">
                      {t.premi.list[1].sottotitolo}
                    </p>

                    {/* Single photo */}
                    <div className="overflow-hidden rounded-sm mb-5">
                      <img
                        src="/images/world-steak-challenge.webp" alt="World Steak Challenge 2022 — MFC Carni"
                        className="w-full h-[200px] object-cover"
                        width={600} height={200}
                        loading="lazy" decoding="async"
                        data-src="/images/world-steak-challenge.webp" onError={onImgError}
                      />
                    </div>

                    <p className="text-osso-parchment/60 text-sm leading-relaxed">{t.premi.list[1].desc}</p>
                  </div>
                </motion.div>
              ...
              <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center max-w-5xl mx-auto">
                <motion.div className="relative"
                  ...
                  <div className="relative overflow-hidden rounded-sm">
                    ...
                    <img
                      src="/images/tbone-superior.webp" alt="T-bone superior frollato"
                      className="w-full h-[260px] sm:h-[360px] md:h-[420px] object-cover"
                      width={800} height={500}
                      loading="eager" decoding="async"
                      data-src="/images/tbone-superior.webp"
                      onLoad={() => setTboneLoaded(true)}
                      onError={onImgError}
                    />
